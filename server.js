...
var dcert = fs.readFileSync("./keys/CA.der.crt");
var mcert = fs.readFileSync("./keys/CA.crt"); // for Https server
var mkey = fs.readFileSync("./keys/priv_and_pub.key"); //password is 1234 for the cert
...
var Merchant_address = "n1s7DwSZ4nSh471iPhAGZZbXkiTvtQCi5B"; //"mhc5YipxN6GhRRXtgakRBjrNUCbz6ypg66";
...
app.get("/request", urlencodedParser, function(req, res) {
  ...
  ); // you can use instead https://...:8883
...
}); ////support parsing of PaymentProtocol.PAYMENT_CONTENT_TYPE-urlencoded post data
...
app.post("/payment", rawBodyParser, function(req, res) {
  ...
  var merchant_data = payment.get(
    "merchant_data"
  ); /*Arbitrary data that may be used by the merchant to identify the PaymentRequest. May be omitted if the merchant does not need to associate Payments with PaymentRequest or if they associate each PaymentRequest with a separate payment address.*/
  var refund_to = payment.get(
    "refund_to"
  ); /*One or more outputs where the merchant may return funds, if necessary. The merchant may return funds using these outputs for up to 2 months after the time of the payment request.*/
  var memo = payment.get("memo");

  var Rawtransaction = payment
    .get("transactions")[0]
    .toBuffer(); /*One or more valid, signed Bitcoin transactions that fully pay the PaymentRequest*/
  var TransactionToBrodcast = new bitcore_lib.Transaction(
    Rawtransaction
  ).toString("hex");
  ...
  rp(Sendingoptions).then(function(response) {
    ...
    ack.set(
      ...
      "Payment processed,Thank you ;) \ninvoice ID :" +
        ...
        "\nTransaction Details : " +
        Jresponse
    ); //invoice is is staticaly defined in payment_url
